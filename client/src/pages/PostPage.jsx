...
PostPage() {
  ...
  async function fetchPostInfo() {
    ...
    try {
      const response = await fetch(`${VITE_API_BACKEND_URL}/posts/${id}`);
      if (!response.ok) {
  ...
  async function handleLike() {
    ...
    try {
      const response = await fetch(
        `${VITE_API_BACKEND_URL}/posts/${id}/likeStatus?action=${status}`,
        {
          method: "POST",
          credentials: "include",
        }
        ...
        <img
          src={postInfo.cover ?
               (postInfo.cover.startsWith('http') ?
                postInfo.cover :
                `${VITE_API_BACKEND_URL.replace('/api', '')}${postInfo.cover}`)
